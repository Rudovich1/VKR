...
def double_none_except(item1, item2) -> None:
    if item1 is None and item2 is None:
        raise HTTPException(status.HTTP_400_BAD_REQUEST, "one item must by not None")
...
def to_list_contracts(
    models: list[MODEL], contract_type: Type[CONTRACT]
) -> list[CONTRACT]:
...
def get_model(
    session: Session, model: int | MODEL | str, model_type: Type[MODEL]
) -> MODEL:
    if type(model) == int:
...
def get_function_param(
    session: Session, param: int | models.FunctionParam, contract_type: Type[CONTRACT]
) -> CONTRACT:
...
def get_function_params(
    session: Session, fun: int | models.Function | str, contract_type: Type[CONTRACT]
) -> list[CONTRACT]:
...
def post_function_params(
    session: Session,
    fun: int | models.Function | str,
    params: list[contract.BaseFunctionParam],
) -> list[contract.Id]:
    fun = get_model(session, fun, models.Function)
    nparam = (
        session.query(models.FunctionParam)
        .filter(
            models.FunctionParam.function_id == fun.id,
            models.FunctionParam.name.in_([param.name for param in params]),
        )
        .all()
    )
    if len(nparam) == 0:
...
def delete_function_params(session: Session, fun: int | models.Function | str) -> None:
...
def get_global_var_arg(
    session: Session, arg: int | models.GlobalVarArg, contract_type: Type[CONTRACT]
) -> CONTRACT:
...
def get_global_var_args(
    session: Session, global_var: int | models.GlobalVar, contract_type: Type[CONTRACT]
) -> list[CONTRACT]:
...
def post_global_var_args(
    session: Session,
    global_var: int | models.GlobalVar,
    args: list[contract.BaseGlobalVarArg],
) -> list[contract.Id]:
    global_var = get_model(session, global_var, models.GlobalVar)
...
def delete_global_var_args(
    session: Session, global_var: int | models.GlobalVar
) -> None:
    global_var_id = get_model_id(session, global_var, models.GlobalVar)
    session.query(models.GlobalVarArg).filter(
        models.GlobalVarArg.global_var_id == global_var_id
    ).delete()
    session.flush()
...
def get_library(
    session: Session, lib: int | models.Library, contract_type: Type[CONTRACT]
) -> CONTRACT:
...
def get_libraries(
    session: Session, program: int | models.Program | str, contract_type: Type[CONTRACT]
) -> list[CONTRACT]:
...
def post_library(
    session: Session, program: int | models.Program | str, lib: contract.BaseLibrary
) -> contract.Id:
    program = get_model(session, program, models.Program)
    nlib = (
        session.query(models.Library)
        .filter(
            models.Library.program_id == program.id, models.Library.name == lib.name
        )
        .first()
    )
...
def post_libraries(
    ...
    program = get_model(session, program, models.Program)
    nlibs = (
        session.query(models.Library.name)
        .filter(
            models.Library.program_id == program.id,
            models.Library.name.in_([lib.name for lib in libs]),
        )
        .all()
    )
    if len(nlibs) == 0:
...
def delete_library(session: Session, library: int | models.Library) -> None:
    lib = get_model(session, library, models.Library)
    session.delete(lib)
    session.flush()


def delete_libraries(session: Session, program: int | models.Program | str) -> None:
    program_id = get_model_id(session, program, models.Program)
    session.query(models.Library).filter(
        models.Library.program_id == program_id
    ).delete()
    session.flush()
...
def get_function_node_arg(
    session: Session, arg: int | models.Function_NodeArg, contract_type: Type[CONTRACT]
) -> CONTRACT:
...
def post_function_node_args(
    session: Session,
    fun_node: int | models.Function_Node,
    args: list[contract.BaseFunction_NodeArg],
) -> list[contract.Id]:
    fun_node = get_model(session, fun_node, models.Function_Node)
    fun = get_model(session, fun_node.function_id, models.Function)
    if len(fun._params) != len(args):
        raise HTTPException(
            status.HTTP_400_BAD_REQUEST, "num params not equal num args"
        )
...
def delete_function_node_args(
    session: Session, fun_node: int | models.Function_Node
) -> None:
    fun_node_id = get_model_id(session, fun_node, models.Function_Node)
    session.query(models.Function_NodeArg).filter(
        models.Function_NodeArg.function_node_id == fun_node_id
    ).delete()
    session.flush()
...
def post_global_var(
    session: Session,
    program: int | models.Program | str,
    global_var: contract.BaseGlobalVar,
) -> contract.Id:
    program = get_model(session, program, models.Program)
    nglobal_var = (
        session.query(models.GlobalVar)
        .filter(
            models.GlobalVar.program_id == program.id,
            models.GlobalVar.name == global_var.name,
        )
        .first()
    )
...
def post_global_vars(
    session: Session,
    program: int | models.Program | str,
    global_vars: list[contract.BaseGlobalVar],
) -> list[contract.Id]:
    program = get_model(session, program, models.Program)
    nglobal_vars = (
        session.query(models.GlobalVar)
        .filter(
            models.GlobalVar.program_id == program.id,
            models.GlobalVar.name.in_([gv.name for gv in global_vars]),
        )
        .all()
    )
...
def delete_global_var(session: Session, global_var: int | models.GlobalVar) -> None:
    global_var = get_model(session, global_var, models.GlobalVar)
    session.delete(global_var)
    session.flush()


def delete_global_vars(session: Session, program: int | models.Program | str) -> None:
    program = get_model(session, program, models.Program)
...
def get_program_graph(
    ...
    program_graph = (
        session.query(models.Program_Graph)
        .filter(
...
def get_program_graphs(
    session: Session,
    contract_type: Type[CONTRACT],
    program: int | models.Program | str | None = None,
    graph: int | models.Graph | str | None = None,
) -> list[CONTRACT]:
...
def post_program_graph(
    ...
    nprogram_graph = (
        ...
        .filter(
            ...
            models.Program_Graph.graph_id == graph.id,
        )
        .first()
    )
...
def delete_program_graph(
    session: Session, program_graph: int | models.Program_Graph
) -> None:
    program_graph = get_model(session, program_graph, models.Program_Graph)
...
def get_function_node(
    ...
    function_node = (
        session.query(models.Function_Node)
        .filter(
...
def post_function_node(
    session: Session, fun: int | models.Function | str, node: int | models.Node | str
) -> contract.Id:
    fun = get_model(session, fun, models.Function)
    ...
    if fun.gene_type != node.gene_type:
        raise HTTPException(
            status.HTTP_400_BAD_REQUEST, "fun gene_type not equal node gene_type"
        )
    nfun = (
        session.query(models.Function)
        .filter(
            models.Function.type == fun.type,
            models.Function.id == models.Function_Node.function_id,
            models.Function_Node.node_id == node.id,
        )
        .first()
    )
    already_exists_except(nfun, f'function with type "{fun.type}" in node')
    if (
        fun.type == contract.FunctionTypes.pooling_populations
        and node.type != contract.NodeTypes.K_node
    ):
        raise HTTPException(
    ...
    if (
        fun.type == contract.FunctionTypes.start_population
        and node.type != contract.NodeTypes.population_node
    ):
        raise HTTPException(
            status.HTTP_400_BAD_REQUEST,
...
def get_node_graph(
    session: Session, node_graph: int | models.Node_Graph, contract_type: Type[CONTRACT]
) -> CONTRACT:
...
def get_node_graph(
    ...
    graph_id = get_model_id(session, graph, models.Graph)
    node_graph = (
        session.query(models.Node_Graph)
        .filter(
            models.Node_Graph.node_id == node_id, models.Node_Graph.graph_id == graph_id
        )
        .first()
    )
...
def post_node_graph(
    ...
    node: int | models.Node | str,
    graph: int | models.Graph | str,
    parent: int | models.Node_Graph | str | None = None,
) -> contract.Id:
    node = get_model(session, node, models.Node)
    graph = get_model(session, graph, models.Graph)
    ...
    if parent is None:
        root = node_graphs_q.filter(
            models.Node_Graph.parent_node_graph_id is not None
        ).first()
        ...
        if parent.graph_id != graph.id:
            raise HTTPException(status.HTTP_400_BAD_REQUEST, "parent node not in graph")
        if parent._node.type == contract.NodeTypes.population_node:
            raise HTTPException(
                status.HTTP_400_BAD_REQUEST, "population node cannot have child"
            )
        if (
            parent._node.type == contract.NodeTypes.unary_node
            and len(parent._child_node_graphs) == 1
        ):
            raise HTTPException(
                status.HTTP_400_BAD_REQUEST, "unary node already has child"
            )
        nnode_graph = node_graphs_q.filter(
            models.Node_Graph.name == node_graph.name
        ).first()
...
def delete_node_graph(session: Session, node_graph: int | models.Node_Graph) -> None:
    node_graph = get_model(session, node_graph, models.Node_Graph)
    session.delete(node_graph)
    session.flush()


def delete_node_graphs(
    session: Session,
    node: int | models.Node | str | None = None,
    graph: int | models.Graph | str | None = None,
) -> None:
...
def get_functions(
    ...
    if type is not None:
        if not isinstance(type, contract.FunctionTypes):
            raise HTTPException(
                status.HTTP_400_BAD_REQUEST, f'"{type}" not function type'
            )
...
def post_function(session: Session, fun: contract.BaseFunction) -> contract.Id:
    nfun = (
        session.query(models.Function).filter(models.Function.name == fun.name).first()
    )
...
def delete_function(session: Session, fun: int | models.Function | str) -> None:
    fun = get_model(session, fun, models.Function)
    session.delete(fun)
    session.flush()
...
def get_node(
    session: Session, node: int | models.Node | str, contract_type: Type[CONTRACT]
) -> CONTRACT:
    node = get_model(session, node, models.Node)
...
def get_nodes(
    ...
    if type is not None:
        if not isinstance(type, contract.NodeTypes):
            raise HTTPException(
                status.HTTP_400_BAD_REQUEST, f'"{type} is not node type"'
            )
    ...
    if function is not None:
        function_id = get_model_id(session, function, models.Function)
        q = q.filter(
            models.Node.id == models.Function_Node.node_id,
            models.Function_Node.function_id == function_id,
        )
    if graph is not None:
        graph_id = get_model_id(session, graph, models.Graph)
        q = q.filter(
            models.Node.id == models.Node_Graph.node_id,
...
def post_node(session: Session, node: contract.BaseNode) -> contract.Id:
    nnode = session.query(models.Node).filter(models.Node.name == node.name).first()
...
def delete_node(session: Session, node: int | models.Node | str) -> None:
    node = get_model(session, node, models.Node)
    session.delete(node)
    session.flush()
...
def get_graph(
    session: Session, graph: int | models.Graph | str, contract_type: Type[CONTRACT]
) -> CONTRACT:
    graph = get_model(session, graph, models.Graph)
...
def get_graphs(
    ...
    program: int | models.Program | str | None = None,
    node: int | models.Node | str | None = None,
    node_graph: int | models.Node_Graph | None = None,
) -> list[CONTRACT]:
    q = session.query(models.Graph)
    if program is not None:
        program_id = get_model_id(session, program, models.Program)
        q = q.filter(
            models.Graph.id == models.Program_Graph.graph_id,
            models.Program_Graph.program_id == program_id,
        )
    if node is not None:
        node_id = get_model_id(session, node, models.Node)
        q = q.filter(
            models.Graph.id == models.Node_Graph.graph_id,
...
def post_graph(session: Session, graph: contract.BaseGraph) -> contract.Id:
    ngraph = session.query(models.Graph).filter(models.Graph.name == graph.name).first()
...
def delete_graph(session: Session, graph: int | models.Graph | str) -> None:
    graph = get_model(session, graph, models.Graph)
...
def get_programs(
    ...
    if graph is not None:
        graph_id = get_model_id(session, graph, models.Graph)
        q = q.filter(
            models.Program.id == models.Program_Graph.program_id,
            models.Program_Graph.graph_id == graph_id,
        )
...
def post_program(session: Session, program: contract.BaseProgram) -> contract.Id:
    nprog = (
        session.query(models.Program)
...
def delete_program(session: Session, program: int | models.Program | str) -> None:
    program = get_model(session, program, models.Program)
...
def _make_graph_from_root(
    node_graph: models.Node_Graph,
) -> tuple[graph_gen.Node, dict[str, header_gen.Function]]:
    node = node_graph._node

    funs = [
        (fun_node._function, fun_node._args) for fun_node in node._function_associations
    ]
    data = {}
    used_functions = {}
    ...
    if node.type == contract.NodeTypes.population_node:
        res_node = graph_gen.PopulationNode(
            id=node_graph.name, gene_type=node.gene_type, **data
        )
    elif node.type == contract.NodeTypes.unary_node:
        if len(next_nodes) == 0:
            raise HTTPException(status.HTTP_400_BAD_REQUEST, "incomplete graph")
        next_node, used_functions = _make_graph_from_root(next_nodes[0])
        res_node = graph_gen.UnaryNode(
            id=node_graph.name, gene_type=node.gene_type, next_node=next_node**data
        )
    else:
        if len(next_nodes) == 0:
            raise HTTPException(status.HTTP_400_BAD_REQUEST, "incomplete graph")
        ...
        res_node = graph_gen.K_Node(
            id=node_graph.name, gene_type=node.gene_type, next_nodes=next_nodess, **data
        )
    for fun in funs:
...
def gen_program(session: Session, program: int | models.Program | str) -> str:
    program = get_model(session, program, models.Program)
    libs = [header_gen.Library(lib.name, lib.is_stl) for lib in program._libraries]
    vars = [
        header_gen.Variable(
            var.type, var.name, [arg.arg for arg in var._args], var.is_const
        )
    ...
    for graph in program._graphs:
        root = (
            ...
            .filter(
                models.Node_Graph.graph_id == graph.id,
                (models.Node_Graph.parent_node_graph_id.__eq__(None)),
            )
            .first()
        )
