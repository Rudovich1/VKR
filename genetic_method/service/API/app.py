...
def get_function(function_name: str) -> contract.FunctionRes:
    ...
    try:
        ...
        res = contract.FunctionRes(
            params=controler.get_function_params(
                session, fun.id, contract.BaseFunctionParam
            ),
            **fun.model_dump()
        )
...
) -> list[contract.FunctionRes]:
    session = controler.get_session()
    try:
        funs = controler.get_functions(
            session, contract.Function, node_name, type, gene_type
        )
        res = [
...
@app.post("/functions")
def post_function(
    function: contract.BaseFunction, params: list[contract.BaseFunctionParam]
) -> None:
    session = controler.get_session()
    try:
...
def get_node(node_name: str) -> contract.NodeRes:
    ...
    try:
        ...
        for fun in functions:
            fun_node = controler.get_function_node(
                session, fun.id, node.id, contract.Function_Node
            )
            fun_params = controler.get_function_params(
                session, fun.id, contract.BaseFunctionParam
            )
            fun_args = controler.get_function_node_args(
                session, fun_node.id, contract.BaseFunction_NodeArg
            )
            res.functions.append(
                contract.FunctionWithArgs(
                    **fun.model_dump(), params=fun_params, args=fun_args
                )
            )
    except Exception as ex:
        session.rollback()
...
) -> list[contract.NodeRes]:
    session = controler.get_session()
    try:
        nodes = controler.get_nodes(
            session, contract.Node, type, gene_type, function_name, graph_name
        )
...
def get_graph(graph_name: str) -> contract.GraphRes:
    ...
    try:
        ...
        node_graphs = controler.get_node_graphs(
            session, contract.Node_Graph, graph=graph.id
        )
        ng_res = []
        for node_graph in node_graphs:
            model = contract.Node_GraphRes(
                name=node_graph.name,
                parent_node_graph_name=(
                    controler.get_node_graph(
                        session, node_graph.parent_node_graph_id, contract.Node_Graph
                    ).name
                    if node_graph.parent_node_graph_id is not None
                    else None
                ),
                node_name=controler.get_node(
                    session, node_graph.node_id, contract.Node
                ).name,
            )
            ng_res.append(model)
...
@app.get("/graphs")
def get_graphs(
    program_name: str | None = None, node_name: str | None = None
) -> list[contract.GraphRes]:
    session = controler.get_session()
    try:
...
def get_program(program_name: str) -> contract.ProgramRes:
    ...
    try:
        ...
        global_vars = controler.get_global_vars(
            session, program_name, contract.GlobalVar
        )
        global_vars_res = [
            contract.GlobalVarRes(
                **global_var.model_dump(),
                args=controler.get_global_var_args(
                    session, global_var.id, contract.BaseGlobalVarArg
                )
            )
...
@app.post("/programs")
def post_program(
    program: contract.BaseProgram,
    libs: list[contract.BaseLibrary],
    global_vars: list[contract.GlobalVarWithArgs],
) -> None:
    session = controler.get_session()
    try:
        ...
        for index in range(len(gv_ids)):
            controler.post_global_var_args(
                session, gv_ids[index].id, gv_args_base[index]
            )
    except Exception as ex:
        session.rollback()
        raise ex
...
def post_connect_program_graph(program_name: str, graph_name: str) -> None:
    session = controler.get_session()
    try:
...
def delete_connect_program_graph(program_name: str, graph_name: str) -> None:
    session = controler.get_session()
    try:
        pg_model = controler.get_program_graph(
            session, program_name, graph_name, contract.Program_Graph
        )
...
def post_connect_function_node(
    function_name: str, node_name: str, args: list[contract.BaseFunction_NodeArg]
) -> None:
    session = controler.get_session()
    try:
...
def delete_connect_function_node(function_name: str, node_name: str) -> None:
    session = controler.get_session()
    try:
        fun_node = controler.get_function_node(
            session, function_name, node_name, contract.Function_Node
        )
...
def post_connect_node_graph(
    ...
    session = controler.get_session()
    try:
        controler.post_node_graph(
            session, node_graph, node_name, graph_name, parent_node_name
        )
    except Exception as ex:
        session.rollback()
        raise ex
...
def delete_connect_node_graph(node_name: str, graph_name: str) -> None:
    session = controler.get_session()
    try:
        node_graph = controler.get_node_graph(
            session, node_name, graph_name, contract.Node_Graph
        )
