...
class FunctionParam(Base, Id):
    ...
    function_id: Mapped[int] = mapped_column(
        ForeignKey("Function.id", ondelete="CASCADE")
    )
...
class GlobalVarArg(Base, Id):
    ...
    global_var_id: Mapped[int] = mapped_column(
        ForeignKey("GlobalVar.id", ondelete="CASCADE")
    )

    _global_var: Mapped["GlobalVar"] = relationship(back_populates="_args")

    def make_model(data: dict[str,]) -> "GlobalVarArg":
        return GlobalVarArg(arg=data["arg"])

    def to_dict(self) -> dict[str,]:
        res = super().to_dict()
        res.update({"arg": self.arg, "global_var_id": self.global_var_id})
        return res
...
class Library(Base, Id):
    ...
    def make_model(data: dict[str,]) -> "Library":
        return Library(name=data["name"], is_stl=data["is_stl"])

    def to_dict(self) -> dict[str,]:
        res = super().to_dict()
        res.update(
            {"name": self.name, "is_stl": self.is_stl, "program_id": self.program_id}
        )
        return res
...
class Function_NodeArg(Base, Id):
    ...
    arg: Mapped[str] = mapped_column()
    function_node_id: Mapped[int] = mapped_column(
        ForeignKey("Function_Node.id", ondelete="CASCADE")
    )

    _function_node: Mapped["Function_Node"] = relationship(back_populates="_args")

    def make_model(data: dict[str,]) -> "Function_NodeArg":
        return Function_NodeArg(arg=data["arg"])
...
class GlobalVar(Base, Id):
    ...
    _args: Mapped[list["GlobalVarArg"]] = relationship(
        back_populates="_global_var", passive_deletes=True, cascade="all, delete"
    )

    def make_model(data: dict[str,]) -> "GlobalVar":
        return GlobalVar(
            name=data["name"], type=data["type"], is_const=data["is_const"]
        )
...
class Program_Graph(Base, Id):
    ...
    def to_dict(self) -> dict[str,]:
        res = super().to_dict()
        res.update({"program_id": self.program_id, "graph_id": self.graph_id})
        return res
...
class Function_Node(Base, Id):
    ...
    _args: Mapped[list["Function_NodeArg"]] = relationship(
        back_populates="_function_node", passive_deletes=True, cascade="all, delete"
    )
...
class Node_Graph(Base):
    ...
    id: Mapped[int] = mapped_column(primary_key=True, unique=True, autoincrement=True)
    name: Mapped[str] = mapped_column()
    parent_node_graph_id: Mapped[int] = mapped_column(
        ForeignKey("Node_Graph.id", ondelete="CASCADE"), nullable=True
    )
    node_id: Mapped[int] = mapped_column(ForeignKey("Node.id", ondelete="CASCADE"))
    graph_id: Mapped[int] = mapped_column(ForeignKey("Graph.id", ondelete="CASCADE"))

    _parent_node_graph: Mapped["Node_Graph"] = relationship(
        back_populates="_child_node_graphs", remote_side=[id]
    )
    _child_node_graphs: Mapped[list["Node_Graph"]] = relationship(
        back_populates="_parent_node_graph", passive_deletes=True, cascade="all, delete"
    )
    ...
    def make_model(data: dict[str,]) -> "Node_Graph":
        return Node_Graph(name=data["name"])
...
class Function(Base, Id):
    ...
    _nodes: Mapped[list["Node"]] = relationship(
        secondary="Function_Node", back_populates="_functions", viewonly=True
    )
    _params: Mapped[list["FunctionParam"]] = relationship(
        back_populates="_function", passive_deletes=True, cascade="all, delete"
    )

    _node_associations: Mapped[list["Function_Node"]] = relationship(
        back_populates="_function", passive_deletes=True, cascade="all, delete"
    )
...
class Node(Base, Id):
    ...
    _functions: Mapped[list["Function"]] = relationship(
        secondary="Function_Node", back_populates="_nodes", viewonly=True
    )
    _graphs: Mapped[set["Graph"]] = relationship(
        secondary="Node_Graph", back_populates="_nodes", viewonly=True
    )
    _node_graphs: Mapped[list["Node_Graph"]] = relationship(
        back_populates="_node", passive_deletes=True, cascade="all, delete"
    )

    _function_associations: Mapped[list["Function_Node"]] = relationship(
        back_populates="_node", passive_deletes=True, cascade="all, delete"
    )

    def make_model(data: dict[str,]) -> "Node":
        return Node(name=data["name"], type=data["type"], gene_type=data["gene_type"])
...
class Graph(Base, Id):
    ...
    _programs: Mapped[list["Program"]] = relationship(
        secondary="Program_Graph", back_populates="_graphs", viewonly=True
    )
    _nodes: Mapped[set["Node"]] = relationship(
        secondary="Node_Graph", back_populates="_graphs", viewonly=True
    )
    _node_graphs: Mapped[list["Node_Graph"]] = relationship(
        back_populates="_graph", passive_deletes=True, cascade="all, delete"
    )

    _program_associations: Mapped[list["Program_Graph"]] = relationship(
        back_populates="_graph", passive_deletes=True, cascade="all, delete"
    )

    def make_model(data: dict[str,]) -> "Graph":
        return Graph(name=data["name"])
...
class Program(Base, Id):
    ...
    _libraries: Mapped[list["Library"]] = relationship(
        back_populates="_program", passive_deletes=True, cascade="all, delete"
    )
    _global_vars: Mapped[list["GlobalVar"]] = relationship(
        back_populates="_program", passive_deletes=True, cascade="all, delete"
    )
    _graphs: Mapped[list["Graph"]] = relationship(
        secondary="Program_Graph", back_populates="_programs", viewonly=True
    )

    _graph_associations: Mapped[list["Program_Graph"]] = relationship(
        back_populates="_program", passive_deletes=True, cascade="all, delete"
    )

    def make_model(data: dict[str,]) -> "Program":
        return Program(name=data["name"])
