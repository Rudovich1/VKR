...
class K_Node(Node):
    ...
    def gen(self) -> str:
        return f"{''.join(map(lambda x: x.gen(), self.next_nodes))}\
...
class PopulationNode(Node):
    ...
    def __init__(self, start_population: tuple[str, list[str]], **kwargs):
        super().__init__(kwargs)
        self.start_population = start_population

    def gen(self) -> str:
        return f"std::unique_ptr<{GAE}PopulationNode<{self.gene_type}>> {self.id} = \
